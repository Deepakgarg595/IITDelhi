...
import static com.app.iitdelhicampus.constants.Constants.USER_LOGIN_MANAGER;
...
import com.app.iitdelhicampus.model.LoginManagerModel;
...
public class LoginWithNumberManagerActivity extends BaseActivity implements View.OnClickListener, OnUpdateResponse {
...
//            showDeviceTokenAlertDialog(this, "", "Device token is not updated, Please refresh token.");
            try {
                FirebaseInstanceId.getInstance().getInstanceId().addOnSuccessListener(LoginWithNumberManagerActivity.this, new OnSuccessListener<InstanceIdResult>() {
                    @Override
...
//            DataStatic.getInstance().updateFCMToken();
    ...
    private void verifiedSuccess() {
        ...
        Intent intent = new Intent(LoginWithNumberManagerActivity.this, clz);
...
//                preferences.setPhone(Utility.replaceFirstZero(phoneNumber.getText().toString().trim()));
                dialog.dismiss();
                Intent intent = new Intent(LoginWithNumberManagerActivity.this, VerificationActivity.class);
                startActivity(intent);
...
//                            pbar.setVisibility(View.VISIBLE);
//                            Intent intent = new Intent(LoginWithNumberManagerActivity.this, OTPVerificationActivity.class);
//                            startActivity(intent);
...
//                    }
            ...
            } else if (isSuccess && responseCode == Constants.RequestType.USER_LOGIN_MANAGER) {
                LoginManagerModel employeeModel = gson.fromJson(response, LoginManagerModel.class);
                if (employeeModel != null) {
                    if (employeeModel.getSuccess().isEmpty()) {
                        showAlertDialog(this, "", "Number does not exist.");
                        return;
                    }

//                    for (EmployeeModel.EmpList empD : employeeModel.getSuccess()) {

//                        if (!(preferences.getDeviceId()+"_"+preferences.getPhone()).equalsIgnoreCase(empD.getDeviceId())) {
//                            showAlertDialog(this, "", "Your Number and deviceId did not mach, Please contact to Admin");
//                            return;
//                        }


                        preferences.setName(employeeModel.getSuccess().get(0).getUserName());
                        preferences.setMobile(employeeModel.getSuccess().get(0).getMobile());
                        preferences.setRoleId(employeeModel.getSuccess().get(0).getRoleId());
                        preferences.setRoleName(employeeModel.getSuccess().get(0).getRoleName());
//                        preferences.setUserType("client");
//                    preferences.setEmail(empD.getEmail());
//                        preferences.setEmpCode(empD.getEmployeeCode());
                        preferences.setUserId(employeeModel.getSuccess().get(0).getUserId());
//                        preferences.setStaticLatitude(empD.getLatitude());
//                        preferences.setStaticLongitude(empD.getLongitude());
//                        preferences.setWarehouseCode(empD.getWarehouseId());
//                        preferences.setRadiusInMeter(empD.getRadius());
//                    preferences.setTeamLead(empD.getTeamlead());
//                    preferences.setTeamLeadID(empD.getTeam_lead_id());
//                        preferences.setDepartment(empD.getLocationName());
                        pbar.setVisibility(View.VISIBLE);
                        Intent intent = new Intent(LoginWithNumberManagerActivity.this, OTPVerificationActivity.class);
                        startActivity(intent);
...
//                            Toast.makeText(LoginWithNumberActivity.this, "Verification Success", Toast.LENGTH_LONG).show();
                            ...
                            if (task.getException() instanceof FirebaseAuthInvalidCredentialsException) {
                                Toast.makeText(LoginWithNumberManagerActivity.this, "Verification Failed, Invalid credentials", Toast.LENGTH_LONG).show();
                            }
                            etOtpHolder.getText().clear();
                            btnVerifyOTP.setEnabled(true);
                            pbar.setVisibility(View.GONE);
...
//                Toast.makeText(LoginWithNumberActivity.this, "Verification Completed.", Toast.LENGTH_LONG).show();
            ...
            public void onVerificationFailed(FirebaseException e) {
                Toast.makeText(LoginWithNumberManagerActivity.this, e.getMessage(), Toast.LENGTH_LONG).show();
                btnVerifyOTP.setEnabled(true);
                btnVerifyNumber.setEnabled(true);
                pbar.setVisibility(View.GONE);
                etOtpHolder.getText().clear();
            ...
            public void onCodeSent(String verificationId,
                ...
                Toast.makeText(LoginWithNumberManagerActivity.this, "Verification Code Sent.", Toast.LENGTH_LONG).show();
                pbar.setVisibility(View.GONE);
                        ...
                        if (task.isSuccessful()) {
                            if (task.getResult().size() > 0) {
                                for (QueryDocumentSnapshot document : task.getResult()) {
                                    ...
                                    LoopJRequestHandler.getInstance().hitApiGetMethod(Constants.CHECK_USER + preferences.getPhone(), LoginWithNumberManagerActivity.this, Constants.RequestType.SEND_OTP);
    ...
    public void hitApiUserLogin() {
        try {
            ...
            try {
                ...
                requestParams.put("fcmToken", preferences.getDeviceToken());
                requestParams.put("DeviceId", preferences.getDeviceId());

            } catch (Exception e) {
                e.printStackTrace();
            }
            LoopJRequestHandler.getInstance().hitApiPostMethodByJSON(USER_LOGIN_MANAGER, requestParams, this, Constants.RequestType.USER_LOGIN_MANAGER);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
